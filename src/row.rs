...
impl Row {
    ...
    pub fn insert(&mut self, at: usize, c: char) {
        if at >= self.len() {
            ...
            // collect is very powerful and can collect into different collections. Since there are
            // multiple collections collect can create, we have to provide the type of result and
            // remainder, otherwise Rust wouldn’t know what kind of collection to create
            //
    ...
    pub fn split(&mut self, at: usize) -> Self {
        ...
        let mut splitted_row_length = 0;
        for (index, grapheme) in self.string[..].graphemes(true).enumerate() {
            if index < at {
                length += 1;
                row.push_str(grapheme);
            } else {
                splitted_row_length += 1;
    ...
    pub fn highlight(
        ...
        let mut index = 0;
        let mut in_ml_comment = starts_with_comment;
        if in_ml_comment {
